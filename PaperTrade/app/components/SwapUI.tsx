...
export function SwapUI({
    ...
    setPosition: (pos: { price: number; quantity: number } | null) => void;
}) {
    ...
    const handleBuy = () => {
        ...
        setPosition({ price: currentPrice, quantity: Number(amount) });
    };

    const handleSell = () => {
        if (!position) return;
        const pnl = (currentPrice - position.price) * position.quantity;
        alert(`You sold for a PnL of ${pnl.toFixed(2)}`);
        setPosition(null);
    };
                    ...
                    <div className="flex flex-col px-3">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-end flex-row">
                                <p className="font-medium pr-2 text-xs text-baseTextMedEmphasis">≈ {(Number(amount) * currentPrice || 0).toFixed(2)} USDC</p>
                            </div>
                            <div className="flex justify-center flex-row mt-2 gap-3">
                                <div className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3"
                                    onClick={() => setAmount(((0.25 * 36.94) / currentPrice).toFixed(4))}>
                                    25%
                                </div>
                                <div className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3"
                                    onClick={() => setAmount(((0.50 * 36.94) / currentPrice).toFixed(4))}>
                                    50%
                                </div>
                                <div className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3"
                                    onClick={() => setAmount(((0.75 * 36.94) / currentPrice).toFixed(4))}>
                                    75%
                                </div>
                                <div className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3"
                                    onClick={() => setAmount((36.94 / currentPrice).toFixed(4))}>
                                    Max
                                </div>
                            </div>
                        </div>
...
function LimitButton({ type, setType }: { type: string, setType: (type: 'limit' | 'market') => void }) {
    return (
...
function MarketButton({ type, setType }: { type: string, setType: (type: 'limit' | 'market') => void }) {
    return (
...
function BuyButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (tab: 'buy' | 'sell') => void }) {
    return (
        <div className={`flex flex-col mb-[-2px] flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'buy' ? 'border-b-greenBorder bg-greenBackgroundTransparent' : 'border-b-baseBorderMed hover:border-b-baseBorderFocus'}`} onClick={() => setActiveTab('buy')}>
...
function SellButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (tab: 'buy' | 'sell') => void }) {
    return (
        <div className={`flex flex-col mb-[-2px] flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'sell' ? 'border-b-redBorder bg-redBackgroundTransparent' : 'border-b-baseBorderMed hover:border-b-baseBorderFocus'}`} onClick={() => setActiveTab('sell')}>
